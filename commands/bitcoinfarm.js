        ...
        if (args[0] === 'c' || args[0] === 'compare')
        {
            ...
            else
            {
                ...
                .addFields(
                    ...
                    {name: 'Difference in GPUS:' , value: `${(SelectedGPUSComp - SelectedGPUS)}`},
                    {name: 'Difference in Bitcoins Per Day:', value: `${(BTCPerDayComp - BTCPerDay)}`},
                    {name: 'Difference in Roubles Per Day:', value: `₽${(Math.floor((BTCPerDayComp * Math.floor((PriceValue.data.amount * 0.19)))) - Math.floor((BTCPerDay * Math.floor((PriceValue.data.amount * 0.19))))).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`}
                )
