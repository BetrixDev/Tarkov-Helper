...
export default async (interaction: CommandInteraction, args: { item: string }): Promise<InteractionReplyOptions> => {
    ...
    const fields = messageData.fields || [{ name: '\u200b', value: '\u200b', inline: true }]

    return {
        embeds: [
            new MessageEmbed()
                .setColor(settings.botSettings.color)
                .setTitle(`${item.shortName} Stats - ${FormatPrice(item.lastLowPrice)}`)
                .setDescription(
                    `
...
class ItemData {
    ...
    get SpecificData(): MessageEmbedOptions {
        ...
        if (type == 'armoredrig') {
            ...
            return {
                fields: [
                    ...
                    },
                    {
                        ...
                        value: Round(data.MaxDurability / globals.config.ArmorMaterials[data.ArmorMaterial].Destructibility, '00'),
                        inline: true
                    },
                    {
                        name: 'Speed Penalty',
            ...
            const penalties = [`Sens: ${data.mousePenalty}%`, `Speed: ${data.speedPenaltyPercent}%`, `Ergo: ${data.weaponErgonomicPenalty}`]
            ...
            return {
                fields: [
                    ...
                    },
                    {
                        ...
                        value: `${Number(data.FragmentationChance) * 100}% (${data.MinFragmentsCount}-${data.MaxFragmentsCount})`,
                        inline: true
                    },
            ...
            return {
                fields: [
                    ...
                    { name: 'Min/Max Explosion Distance', value: `${data.MinExplosionDistance}/${data.MaxExplosionDistance}m`, inline: true },
            ...
            return {
                fields: [
                    ...
                    { name: 'Weapon Type', value: data.weapClass[0].toUpperCase() + data.weapClass.substr(1), inline: true }
                ]
            }
        } else if (type == 'helmet') {
            const destructibility = ArmorMaterials[data.ArmorMaterial].Destructibility
            ...
            return {
                fields: [
                    ...
                    { name: 'Grid Size', value: `${item.height * item.width} *(${item.width}x${item.height})*`, inline: true },
                    { name: 'Space Efficiency', value: `${data.Cartridges[0]._max_count / (item.height * item.width)}`, inline: true },
                    { name: 'Ergonomics', value: data.Ergonomics, inline: true },
            ...
            return {
                ...
                image: { url: `https://raw.githubusercontent.com/Tarkov-Helper/Image-Database/main/thermal_ingame/${item.id}.png` }
            }
        } else if (type == 'shell') {
            return {
                fields: [
            ...
            return {
                fields: [
                    ...
                    { name: 'Goggle Color', value: `rgb(${data.Color.r}, ${data.Color.g}, ${data.Color.b})`, inline: true },
